...
class HomeVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for _ in 0..<100 {  // put inside viewDidLoad
...
//        table.backgroundColor = UIColor.withAlphaComponent(lightGray)(0.6)
        table.layer.cornerRadius = 12
        table.register(AlbumTableViewCell.self, forCellReuseIdentifier: AlbumTableViewCell.reuseId)
        //table.register(FavoritesTableViewController.self, forCellReuseIdentifier: FavoritesTableViewController.reuseId)
    ...
    let musicVM: AlbumViewModelCore
  //  let favMusicVM = AlbumViewModel (networkManager: NetworkManager())
    var a: [FavAlbums]?
    init(vm: AlbumViewModelTypes) {
        self.a = vm.findAll()
        self.musicVM = vm as AlbumViewModelCore
        super.init(nibName: nil, bundle: nil)
        self.musicVM.bind {
            DispatchQueue.main.async {
                self.MusicTable.reloadData()
...
extension FavoriteVC: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configureFav(musicVM: self.musicVM, index: indexPath.row) // 18 mins
